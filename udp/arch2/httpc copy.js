...
client.connect(argv.routerport, argv.routerhost, (err) => {
  console.log(
    `Client sent SYN, sequenceNumber:${initilizedSequenceNumber}, expectedSequenceNumber:${expectedSequenceNumber}`
  );
...
client.on("message", (pkt, rinfo) => {
  ...
  switch (udp.getKeyByValue(udp.typeDict, p.type)) {
    case "Data":
      console.log(
        `Client got Data, sequenceNumber:${p.sequenceNumber}, expectedSequenceNumber:${expectedSequenceNumber}`
      );
    ...
    case "SYN_ACK":
      ...
      console.log(
        `Client got SYN_ACK ${ack}, sequenceNumber:${p.sequenceNumber}, expectedSequenceNumber:${expectedSequenceNumber}`
      );
      if (ack == expectedSequenceNumber) {
        ...
        client.send(udp_pkt, (err) => {});
        console.log(`Client sent ACK ${p.sequenceNumber}`);
        ...
        client.send(udp_pkt, (err) => {});
        console.log(
          `Client sent Data, sequenceNumber:${p.sequenceNumber}, expectedSequenceNumber:${expectedSequenceNumber}`
        );
      } else {
        ...
        client.send(udp_pkt, (err) => {});
        console.log(
          `Client sent NAK, sequenceNumber:${p.sequenceNumber}, expectedSequenceNumber:${expectedSequenceNumber}`
        );
      }
      break;
    case "NAK":
